...
from notifications import notify_item_received, notify_death, notify_goal, notify_bulk_release, death_final_count
...
class ArchipelagoMonitor:
	...
	def __init__(self, server_url: str, config: RoomsConfig, on_room_finished=None):
	...
	async def handle_print_json(self, pkt):
		...
		if pkt_type == "Part":
			log.info("[%s] Fin de la surveillance active, %d joueur(s)", self.server_url, len(self.slot_names))
	...
	async def handle_goal(self, pkt):
		...
		if self.config.all_finished(self.server_url):
			if self.on_room_finished is not None:
				await self.on_room_finished(self.server_url)
		#if self.config.all_finished(self.server_url):
		#	await finish_room(self.server_url)
